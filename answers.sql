...
WHERE m.name = 'Mustang' AND m.brand_name = 'Ford';
...
SELECT CARS.id, CARS.year, CARS.brand_name, CARS.name
FROM (SELECT ROW_NUMBER()
OVER (ORDER BY name ASC) AS Row, id, year, brand_name, name
FROM models) AS CARS
WHERE Row BETWEEN 15 AND 24
;
